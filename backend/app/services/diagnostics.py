...
_DOC_META_PATTERNS = [
    ...
    re.compile(r"\bchart\s+\d+", re.IGNORECASE),
    re.compile(r"\bsee\s+(?:the\s+)?(?:figure|table|chart|diagram)\b", re.IGNORECASE),
    re.compile(r"\bin\s+(?:the\s+)?(?:figure|table|chart|section|chapter)\b", re.IGNORECASE),
    re.compile(r"\bpage\s+(?:number|no\.?|#|heading)", re.IGNORECASE),
    re.compile(r"\bchapter\b", re.IGNORECASE),
    re.compile(r"\bmentioned\s+in\b", re.IGNORECASE),
